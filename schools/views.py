...
def classes(request, school_object_id):
    ...
    my_sections = settings.AAD_REQUEST.get_section_by_member(aad.access_token, school_info['school_id'])
...
def users(request, school_object_id):
    ...
    all_teachers, teachersnextlink = settings.AAD_REQUEST.get_teachers_by_schoolid(aad.access_token, school_info['school_id'])
    out_teachers = school_processer.normalize_teachers_info(all_teachers)
    # get students for school from aad graph api
    all_students, studentsnextlink = settings.AAD_REQUEST.get_students_by_schoolid(aad.access_token, school_info['school_id'])
    out_students = school_processer.normalize_students_info(all_students)
    # set parameter for template
    parameter = {}
    parameter['user'] = user_info
