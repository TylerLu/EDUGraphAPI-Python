...
class AADGraphRequest(object):
    ...
    def _get_access_token(self):
        access_token = self._token_service.get_access_token('aad')
        return access_token

    def send(self, url, version='?api-version=1.6'):
    ...
    def get_schoold_uid(self):
        try:
            url = 'me'
            user_content = self.send(url)
            licenses_list = user_content['assignedLicenses']
            for license in licenses_list:
                sku_ids.add(license['skuId'])
        except:
    ...
    def get_all_schools(self):
        ...
        try:
            ...
            schools_content = self.send(url, version)
            schools_list = schools_content['value']
        except:
            pass
        return schools_list
    
    def get_one_school(self, object_id):
        school_result = {}
        try:
            url = 'administrativeUnits/%s' % object_id
            version = '?api-version=beta'
            school_content = self.send(url, version)
            school_result = school_content
        except:
            pass
        return school_result

    def get_sections_by_schoolid(self, schoolid, top=12, nextlink=''):
        skiptoken = ''
        ...
        try:
            ...
            version = ''
            sections_content = self.send(url, version)
    ...
    def get_section_by_member(self, schoolid):
        mysection_list = []
        url = 'me/memberOf'
        version = '?api-version=1.5'
        mysections_content = self.send(url, version)
    ...
    def get_one_section(self, object_id):
        section_result = {}
        try:
            url = 'groups/%s' % object_id
            version = '?api-version=1.5'
            section_content = self.send(url, version)
            section_result = section_content
        except:
            pass
        return section_result

    def get_teachers_for_section(self, object_id):
        teachers_list = []
        try:
            url = 'groups/%s/members' % object_id
            version = '?api-version=1.5'
            teachers_content = self.send(url, version)
    ...
    def get_students_for_section(self, object_id):
        students_list = []
        try:
            url = 'groups/%s/members' % object_id
            version = '?api-version=1.5'
            students_content = self.send(url, version)
    ...
    def get_users_for_school(self, object_id, top=12, nextlink=''):
        skiptoken = ''
        if nextlink and nextlink.find('skiptoken') != -1:
            link_skiptoken = self._token_re.findall(nextlink)[0]
            skiptoken = '&%s' % link_skiptoken
        ...
        try:
            ...
            version = ''
            users_content = self.send(url, version)
    ...
    def get_students_by_schoolid(self, schoolid, top=12, nextlink=''):
        skiptoken = ''
        if nextlink and nextlink.find('skiptoken') != -1:
            link_skiptoken = self._token_re.findall(nextlink)[0]
            skiptoken = '&%s' % link_skiptoken
    ...
    def get_teachers_by_schoolid(self, schoolid, top=12, nextlink=''):
        skiptoken = ''
        if nextlink and nextlink.find('skiptoken') != -1:
            link_skiptoken = self._token_re.findall(nextlink)[0]
            skiptoken = '&%s' % link_skiptoken
